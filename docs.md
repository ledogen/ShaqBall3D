...
- **Launcher windup time** decreases with difficulty: `windup(d) = max(0, 0.8 - d * 0.06)` seconds. At difficulty 0 = 0.8s (current base). Reaches 0 at difficulty ~13, meaning the launcher fires instantly with no warning at high levels.
- **Spinner angular velocity** scales with difficulty: base range of 0.8–2.5 rad/s multiplied by `1 + d * 0.15`. At difficulty 5 spinners spin 1.75× faster; at difficulty 10 they spin 2.5× faster.

#### Stage Time Limit
No time limit on level 1 (tutorial breathing room). From level 2 onward, a countdown timer starts when the level begins. Failing to reach the goal in time ends the run.

Timer formula: `timeLimit(d) = 30 + (d - 1) * 4` seconds, where `d` is the level number starting at 2. Level 2 = 30s, level 3 = 34s, level 4 = 38s, and so on. Stage difficulty grows faster than the timer — higher levels add more obstacles, longer stages, and harder surfaces, so the extra seconds don't keep pace with the added challenge. The net effect is increasing time pressure the further you progress.

Visual: countdown displayed on HUD, turns red and pulses in the final 10 seconds.
